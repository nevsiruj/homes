<template>
  ...
  <div v-if="!isLoading && !notFound" class="max-w-[1080px] mx-auto p-4 bg-white mt-24 lg:mt-12">
    <!-- Breadcrumbs -->
    <Breadcrumbs v-if="inmuebleDetalle" :breadcrumbs="[
      { name: 'Inicio', url: '/' },
    ...
    <div class="grid grid-cols-1 md:grid-cols-6 md:grid-rows-3 gap-8 h-auto md:h-[600px]">
      <div class="md:col-span-5 md:row-span-3 relative">
        ...
        <div v-if="mainMedia.type === 'video'"
          ...
          <template v-if="isYouTubeVideo(mainMedia.url)">
            <iframe :src="getYouTubeEmbedUrl(mainMedia.url)" frameborder="0"
              allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
              allowfullscreen class="w-full h-full object-contain"></iframe>
          </template>
          <template v-else>
            <video :src="mainMedia.url" controls class="w-full h-full object-contain" alt="Video del inmueble"></video>
          </template>
        </div>
        <img v-else :src="mainMedia.url" class="w-full h-auto md:h-[600px] object-cover"
      ...
      <div class="md:col-start-6 p-2 md:p-0 relative">
        <swiper ref="mySwiper" :direction="isMobile ? 'horizontal' : 'vertical'" :slides-per-view="isMobile ? 3 : 4"
          :space-between="8" :mousewheel="!isMobile" :free-mode="true" :loop="shouldEnableLoop"
          :modules="[FreeMode, Mousewheel, Navigation]" class="lg:h-[600px] md:h-[600px] w-full">
          <swiper-slide v-for="(media, index) in allMedia" :key="index"
            ...
            <img v-if="media.type === 'image'" :src="media.url" alt="Miniatura del inmueble"
              class="w-full h-full object-cover" />
            <div v-else class="w-full h-full bg-black flex items-center justify-center">
              <svg xmlns="http://www.w3.org/2000/svg" class="h-12 w-12 text-white absolute" viewBox="0 0 20 20"
                fill="currentColor">
              ...
              " :src="getYouTubeThumbnail(media.url)" class="w-full h-full object-cover opacity-60"
                alt="Video thumbnail" />
              <img v-else-if="inmuebleDetalle.imagenPrincipal" :src="inmuebleDetalle.imagenPrincipal"
                class="w-full h-full object-cover opacity-60" alt="Video thumbnail" />
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 md:grid-rows-1 gap-4">
      <div class="order-first md:order-last md:col-span-1">
        <div class="mb-5 mt-8 md:mt-14 lg:mt-14 space-y-3">
          ...
          <div class="grid grid-cols-2 gap-2 justify-items-center text-sm md:text-base">
            <div class="flex items-center justify-between">
              <svg class="w-5 h-5 me-2" viewBox="0 0 192 192" xmlns="http://www.w3.org/2000/svg" fill="none">
              ...
              <div class="flex flex-col">
                ...
                <NuxtLink v-if="inmuebleDetalle.ubicaciones"
            ...
            <div class="flex items-center justify-between">
              <svg class="w-6 h-6 me-2" fill="#000000" height="200px" width="200px" version="1.1" id="Layer_1"
            ...
            <div class="flex items-center justify-between">
              <svg class="w-6 h-6 me-2" fill="#000000" viewBox="0 0 50 50" xmlns="http://www.w3.org/2000/svg"
          ...
          <div class="mb-4 mt-4 long-text-roboto text-center">
            <ul class="flex flex-wrap items-center justify-center text-gray-900">
              <li v-for="(amenidad, index) in inmuebleDetalle.amenidades" :key="index"
    ...
    <div v-if="!notFound && suggestedProperties && suggestedProperties.length > 0" class="mt-12 mb-12">
      <h1
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        <SugerenciaCard v-for="property in suggestedProperties" :key="property.id" :item="property" type="propiedad" />
      </div>
    </div>
