...
<<<<<<< HEAD
                    <div v-for="blog in paginatedBlogs" :key="`blog-${blog.id}-${blog.Slug}`"
                        ...
                        <NuxtLink :to="getBlogUrl(blog)" class="flex flex-col h-full">
                            <div v-if="getPreviewImage(blog)" class="overflow-hidden rounded-t-lg w-full">
                                <img :src="getPreviewImage(blog)" :alt="blog.Title"
                            ...
                            <div class="flex flex-col flex-1 px-4 pt-4 pb-6 text-center">
                                ...
                                <div v-if="blog['Categorías']" class="text-base text-gray-500 mb-1">
                                    {{ blog['Categorías'] }}
                                </div>
                                ...
                                <div class="mt-auto pt-4">
                                    <span
                                        class="inline-block bg-black hover:bg-gray-900 text-white text-sm px-5 py-2 rounded transition-colors duration-200 font-light"
                                        style="outline: none;">
                                        Seguir viendo
                                    </span>
                                </div>
...
function normalizeCategory(category) {
    ...
    // Normalizar la categoría: minúsculas, sin espacios, sin acentos
    return category
        .toLowerCase()
        .normalize('NFD')
...
function getBlogUrl(blog) {
    const category = normalizeCategory(blog['Categorías'])
