# deploy-medianaranja.ps1
# Script para deployar CRM, API y Web a las ubicaciones de Media Naranja
...
# Configurar log
$LogFile = "C:\DeployScripts\logs\deploy-medianaranja-$(Get-Date -Format 'yyyyMMdd-HHmmss').log"
...
if (-not ($DeployCRM -or $DeployAPI -or $DeployWeb)) {
    Write-Log "ERROR: Debe especificar al menos un componente para deployar (-DeployCRM, -DeployAPI, -DeployWeb, o -All)"
    exit 1
}

try {
    Write-Log "=== Starting Media Naranja Deployment ==="
    ...
    if ($DeployCRM) {
        Write-Log "========================================="
        Write-Log "Deploying CRM to \\vm06\shared\medianaranja"
        Write-Log "========================================="
        
        $crmPath = "\\vm06\shared\medianaranja"
        ...
        if (Test-Path $crmPath) {
            ...
            Get-ChildItem -Path "\\vm06\shared" -Filter "medianaranja-backup-*" | 
...
"@
    ...
    if ($DeployAPI) {
        Write-Log "========================================="
        Write-Log "Deploying API to \\vm06\shared\medianaranja\api"
        Write-Log "========================================="
        
        $apiPath = "\\vm06\shared\medianaranja\api"
...
"@
        ...
        if (Test-Path $apiPath) {
            ...
            Get-ChildItem -Path "\\vm06\shared\medianaranja" -Filter "api-backup-*" | 
            Sort-Object CreationTime -Descending | 
            Select-Object -Skip 3 | 
            Remove-Item -Recurse -Force
        }
    ...
    if ($DeployWeb) {
        Write-Log "========================================="
        Write-Log "Deploying Web to \\vm06\shared\qa\homes\web"
        Write-Log "========================================="
        
        $webPath = "\\vm06\shared\qa\homes\web"
        
        # Navegar al directorio de la Web
        Write-Log "Navigating to Web directory"
        Set-Location "$RepoPath\homes-web"
        ...
        # Actualizar config.js para apuntar a la API de Media Naranja
        Write-Log "Updating config.js to use Media Naranja API"
        $configContent = @"
//config.js
export const API_BASE_URL = 'https://medianaranja.vylaris.online/api'

"@
        $configContent | Out-File -FilePath "config.js" -Encoding UTF8 -Force
        ...
        if ($buildExitCode -ne 0) {
            throw "Web Build failed with exit code $buildExitCode"
        }
        
        Write-Log "Web Build completed successfully"
        
        # Verificar que el build fue exitoso
        if (-not (Test-Path ".output\public\index.html")) {
            throw "Web Build failed - index.html not found in .output\public"
        }
        
        # Crear backup
        if (Test-Path $webPath) {
            $BackupPath = "$webPath-backup-$(Get-Date -Format 'yyyyMMdd-HHmmss')"
            Write-Log "Creating Web backup: $BackupPath"
            Copy-Item -Path $webPath -Destination $BackupPath -Recurse -Force
            
            # Limpiar backups antiguos
            Get-ChildItem -Path "\\vm06\shared\qa\homes" -Filter "web-backup-*" | 
            Sort-Object CreationTime -Descending | 
            Select-Object -Skip 3 | 
            Remove-Item -Recurse -Force
        }
        ...
        if (-not (Test-Path $webPath)) {
            Write-Log "Creating Web directory: $webPath"
            New-Item -ItemType Directory -Path $webPath -Force | Out-Null
        }
        
        # Limpiar directorio actual
        Write-Log "Cleaning Web directory"
        Get-ChildItem -Path $webPath -Recurse | Remove-Item -Recurse -Force -ErrorAction SilentlyContinue
        
        # Copiar archivos del build
        Write-Log "Copying Web build files"
        $BuildPath = "$RepoPath\homes-web\.output\public"
        Copy-Item -Path "$BuildPath\*" -Destination $webPath -Recurse -Force
        
        # Crear archivo de version
        Write-Log "Creating Web version file"
        $gitCommit = try { git -C $RepoPath rev-parse --short HEAD 2>$null } catch { "unknown" }
        $gitBranch = try { git -C $RepoPath rev-parse --abbrev-ref HEAD 2>$null } catch { "unknown" }
        ...
        $versionInfo | ConvertTo-Json | Out-File -FilePath "$webPath\version.json" -Encoding UTF8 -Force
        
        # Crear web.config
        Write-Log "Creating Web web.config"
        $webConfig = @"
<?xml version="1.0" encoding="UTF-8"?>
<configuration>
  <system.webServer>
...
"@
        $webConfig | Out-File -FilePath "$webPath\web.config" -Encoding UTF8 -Force
        
        Write-Log "Web deployment completed successfully"
        
        # Restaurar config.js original
        Write-Log "Restoring original config.js"
        $originalConfig = @"
//config.js
//export const API_BASE_URL = 'https://iisvm09.vylaris.online/homes-crm/api'
export const API_BASE_URL = 'https://app-pool.vylaris.online/homes/api'
// export const API_BASE_URL = 'https://localhost:44301'

"@
        $originalConfig | Out-File -FilePath "$RepoPath\homes-web\config.js" -Encoding UTF8 -Force
    }
    ...
    if ($DeployCRM) { Write-Log "CRM deployed to: \\vm06\shared\medianaranja" }
    if ($DeployAPI) { Write-Log "API deployed to: \\vm06\shared\medianaranja\api" }
    if ($DeployWeb) { Write-Log "Web deployed to: \\vm06\shared\qa\homes\web" }
